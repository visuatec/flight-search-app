...
# Initialize OpenAI client configured for xAI API
client = OpenAI(
    api_key=XAI_API_KEY,
...
def format_data(user_message, retries=3, delay=5, model="grok-4"):
    """
    Send the prompt to the Grok model using the xAI API via OpenAI SDK.
    Retries on transient failures.
    """
    if not XAI_API_KEY:
        raise RuntimeError("Missing xAI API key (env var XAI_API_KEY)")

    max_message_length = 8000  # Adjust if you need bigger chunks
    user_message_chunks = split_message(user_message, max_message_length)

    combined_output = []
    system_message = "You are my travel planning assistant. You find the best flights for me."
    ...
    for chunk in user_message_chunks:
        ...
        for attempt in range(retries):
            try:
                ...
                logger.warning(f"xAI API call failed (attempt {attempt+1}/{retries}): {e}")
                if attempt < retries - 1:
                    time.sleep(delay)
                else:
                    error_msg = str(e)
                    if "model does not exist" in error_msg.lower() or "access" in error_msg.lower():
                        raise ValueError(
                            f"Model '{model}' is not available or your team does not have access. "
                            "Please check your xAI API key and model availability in the xAI Console, "
                            "or contact xAI support with your team ID."
                        ) from e
                    raise
...
def send_prompt():
    try:
        ...
        # Use Grok model via the xAI API
        response_text = format_data(prompt, retries=3, delay=5, model="grok-4")
        summary = response_text.strip()

        return jsonify({"summary": summary}), 200

    except ValueError as e:
        logger.error(f"Error processing prompt with xAI API: {e}")
        return jsonify({"error": str(e)}), 400
    except Exception as e:
        logger.error(f"Error processing prompt with xAI API: {e}")
        return jsonify({"error": "Failed to process data with xAI API"}), 500
...
def search_flights():
    ...
    if not FLIGHT_API_KEY:
        logger.error("Missing FLIGHT_API_KEY")
        return jsonify({"error": "Server misconfiguration: missing FLIGHT_API_KEY"}), 500
    ...
    try:
        ...
        while True:
            url = (
                ...
                f"{FLIGHT_API_KEY}/"
                f"{source_airport}/{destination_airport}/"
                f"{departure_date}/{return_date}/1/0/0/Economy/USD"
            )
            ...
            carriers = {carrier.get("id"): carrier for carrier in data.get("carriers", [])}
            agents = {agent.get("id"): agent for agent in data.get("agents", [])}

            for itinerary in itineraries:
                # Extracting the price
                price_info = itinerary.get("pricing_options", [{}])[0].get("items", [{}])[0]
                ...
                for leg_id in itinerary.get("leg_ids", []):
                    ...
                    # Keep only HH:MM:SS from timestamps if present
                    dep_t = departure_time[:8] if isinstance(departure_time, str) else departure_time
                    arr_t = arrival_time[:8] if isinstance(arrival_time, str) else arrival_time

                    leg_info = f"{origin_place_id}-{destination_place_id}-{dep_t}-{arr_t}--{duration} mins"
                    ...
                    for segment_id in leg.get("segment_ids", []):
                        ...
                        if segment:
                            ...
                            destination_place = places.get(segment.get("destination_place_id"))
                            if origin_place:
                ...
                # Populate agents
                pricing_opts = itinerary.get("pricing_options", [])
                if pricing_opts:
                    for agent_id in pricing_opts[0].get("agent_ids", []):
