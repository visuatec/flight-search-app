...
def format_data(user_message, retries=3, delay=5, model="gpt-4.1"):
    """
    Send the prompt to the latest ChatGPT model using the new SDK.
    Retries on transient failures.
    """
    if not OPENAI_API_KEY:
        raise RuntimeError("Missing OpenAI API key (env var CHATGPT)")
    ...
    for chunk in user_message_chunks:
        ...
        for attempt in range(retries):
            try:
                response = client.chat.completions.create(
                    model=model,  # "gpt-4.1" is the current flagship; consider "gpt-4.1-mini" for cheaper/faster
                    messages=[
        ...
        if response and response.choices:
            ...
            raise ValueError("The OpenAI API response did not contain choices.")

    # Combine all chunks into one response
    full_response = "".join(combined_output)

    # Debugging information: log the raw response
    logger.debug("OpenAI full response: %s", full_response)
...
def send_prompt():
    try:
        ...
        # Use latest ChatGPT model via the new SDK
        response_text = format_data(prompt, retries=3, delay=5, model="gpt-4.1")
        summary = response_text.strip()

        return jsonify({"summary": summary}), 200

    except Exception as e:
        logger.error(f"Error processing prompt with OpenAI: {e}")
